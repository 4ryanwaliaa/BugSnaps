...
export function Services() {
  return (
    <Section id="services">
      ...
      {/* Phones get a horizontal snap carousel (short page, swipe to browse);
          sm+ keeps the original grid. */}
      <RevealGroup className="no-scrollbar -mx-6 mt-12 flex snap-x snap-mandatory gap-4 overflow-x-auto px-6 pb-3 sm:mx-0 sm:mt-16 sm:grid sm:grid-cols-2 sm:gap-5 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-3">
        {services.map((service) => (
          <RevealItem
            key={service.title}
            className="h-full w-[80vw] max-w-[320px] shrink-0 snap-center sm:w-auto sm:max-w-none sm:shrink"
          >
